...
\subsection{Estimation with leverage}

\subsubsection{Markov Chain Monte Carlo algorithms}
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item Draw $\bm\theta\mid\bm{y}^\ast,\bm{d}$\label{enum:draw-theta},
		\item Draw $(\bm{h},\mu)\mid\bm{y}^\ast,\bm{d},\bm\theta$\label{enum:draw-latent}.
	\end{enumerate}
\end{enumerate}
Steps~\ref{enum:draw-s} and~\ref{enum:draw-other} are interchangeable in the algorithm but~\ref{enum:draw-theta} and~\ref{enum:draw-latent} are not because the former's by-products are used in the latter.
