...
\end{alignedat}
\end{equation}
Note that $\tilde{\mu}_t$ is constant through $t$.
Since $(\xi_t,\nu_t,0)$ is a (degenerate) normal white-noise series, setting $\mu\equiv\tilde{\mu}_t$, and assuming a normal prior for $h_1\mid(\mu,\sigma,\phi)$ and $\mu$, model~\eqref{form:gauss_model} is a linear Gaussian state space model with hidden state $(h_t,\tilde{\mu}_t)$.
We need this ``trick'' with the inclusion of $\mu$ in the hidden state in order to match the general form of models that can be fitted with the algorithm in~\citet{de1995simulation}.
We will adapt the priors of the latent state used by~\citeauthor{Omori2007}, namely, for arbitrary constant $\sigma_0$,
\begin{equation*}
\begin{pmatrix}
h_1 \\
\tilde\mu_1
...
After initialising all the variables and parameters, there are three main steps when estimating a conditional linear Gaussian state space such as model~\ref{form:gauss_model}:
\begin{enumerate}
